...
test.beforeAll(async () => {
    server = spawn(
        ...
        ],
        {
            ...
            env: {
                ...process.env,
                MOCK_INTERVAL_SECONDS: '0.1',
                COMMENT_HISTORY_SIZE: '30',
                STATIC_DIR: resolve('build'),
            },
            stdio: 'pipe',
        },
    );
...
async function expectNoHorizontalOverflow(page: Page) {
    expect(
        await page.evaluate(() => {
            const selectors =
                'main, header, .account-panel, .account-panel form, .monitor-actions, .live-summary, footer, dialog[open]';
            return (
...
for (const width of [390, 320]) {
    test(`setup, monitor and keyboard-operated settings fit a ${width}px screen`, async ({
        page,
    }) => {
        ...
        await page.keyboard.press('Enter');
        await expect(page.getByLabel('TikTok 아이디 또는 방송 주소')).toBeFocused();
...
test('invalid input in collapsed advanced settings is revealed and focused', async ({ page }) => {
    ...
    page.on('request', (request) => {
        if (request.method() === 'PATCH' && request.url().endsWith('/config'))
