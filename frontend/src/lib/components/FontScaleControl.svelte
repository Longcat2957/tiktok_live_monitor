<script lang="ts">
    ...
    onMount(() => {
        try {
            ...
            /* Keep the current scale when storage is unavailable. */
        }
    });
    ...
    function adjust(step: number) {
        ...
        try {
            ...
            /* The current scale still works when storage is unavailable. */
        }
    }
</script>
...
<div class="font-scale-control" role="group" aria-label="댓글 글자 크기">
    ...
    <output aria-live="polite" aria-atomic="true"
        ><span class="sr-only">댓글 글자 크기 </span>{value}%</output
    >
