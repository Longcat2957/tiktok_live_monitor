    ...
    await expect
        .poll(
            async () => {
                ...
                try {
                    return (await fetch('http://127.0.0.1:18765/health')).ok;
                } catch {
                    return false;
                }
            },
...
for (const viewport of [
    ...
    test(`mock comments fit ${viewport.width}×${viewport.height} and remain bounded`, async ({
        page,
        request,
    }) => {
        const errors: string[] = [];
        ...
        if (viewport.width === 1080) {
            ...
            await page.keyboard.press('Tab');
            await expect(page.getByRole('radio', { name: '실제 방송' })).toBeFocused();
        ...
        expect(await page.getByRole('button', { name: '시작', exact: true }).boundingBox()).toEqual(
            startBefore,
        );
        ...
        if (viewport.width === 720) {
            ...
            const fontSize = () =>
                page
                    .locator('[data-kind=comment] .body')
                    .last()
            ...
            await expect.poll(fontSize).toBeCloseTo(original * 0.2, 1);
        ...
        for (let i = 0; i < 1000; i++) {
            socket.send(
                JSON.stringify({
                    ...
                    user: { nickname: '', unique_id: 'fallback-id' },
                    comment: `댓글 ${i} ${'a'.repeat(500)}`,
                }),
            );
        }
    });
...
test('account entry, validation, switching and reconnect session boundary', async ({ page }) => {
    ...
    const status = (username: string | null) =>
        JSON.stringify({
            type: 'status',
            source: 'tiktok',
            state: username ? 'connected' : 'idle',
            message: username ? '연결됨' : '계정 입력 대기',
            session_id: String(generation),
            comment_history_size: 30,
            username,
        });
    await page.routeWebSocket('**/ws', (socket) => {
    ...
    await page.route('**/config', async (route) => {
        if (route.request().method() === 'GET') {
            ...
            await route.fulfill({
                json: { ...(await response.json()), session_id: String(generation) },
            });
        } else {
            generation++;
            send(status('first'));
    ...
    await page.route('**/account', async (route) => {
        ...
        send(
            JSON.stringify({
                ...
                received_at: new Date().toISOString(),
                user: { nickname: username, unique_id: username },
                comment: `새 방송 ${username}`,
            }),
        );
    });
    ...
    const applied = page.waitForRequest(
        (r) => r.url().endsWith('/config') && r.method() === 'PATCH',
    );
    await liveDialog.getByRole('button', { name: '적용', exact: true }).click();
    expect((await applied).postDataJSON()).toMatchObject({
        session_id: String(generation - 1),
        settings: { tiktok_reconnect_min_seconds: 20 },
    });
...
test('idle session updates reset drafts without losing or stealing focus', async ({ page }) => {
    let send: (session: string, source: 'tiktok' | 'mock') => void;
    await page.routeWebSocket('**/ws', (socket) => {
        send = (session_id, source) =>
            socket.send(
                JSON.stringify({
                    type: 'status',
                    source,
    ...
    await page.route('**/account', (route) => route.fulfill({ status: 422, json: {} }));
    await page.getByRole('button', { name: '시작', exact: true }).click();
...
test('late account errors cannot overwrite a new idle session', async ({ page }) => {
    let send: (session: string) => void;
    ...
    await page.route('**/account', (route) => {
        respond = () => route.fulfill({ status: 422, json: {} });
    });
    await page.goto('/');
    const input = page.getByLabel('TikTok 아이디 또는 방송 주소');
    await input.fill('old-account');
    await page.getByRole('button', { name: '시작', exact: true }).click();
...
test('toolbar icons show accessible tooltips and reset every monitor', async ({
    page,
    context,
    request,
}) => {
    ...
    const oldIds = await other
        .locator('.comment')
        .evaluateAll((nodes) => nodes.map((n) => n.getAttribute('data-comment-id')));
    ...
    const posted = page.waitForRequest(
        (r) => r.url().endsWith('/refresh') && r.method() === 'POST',
    );
    await refresh.click();
    expect((await posted).postDataJSON()).toEqual({ session_id: before.session_id });
    await expect(page.getByRole('group', { name: '방송 또는 데모' })).toBeHidden();
        ...
        await expect
            .poll(() =>
                monitor
                    ...
                    .evaluateAll(
                        (nodes, ids) =>
                            nodes.some((n) => ids.includes(n.getAttribute('data-comment-id'))),
                        oldIds,
                    ),
            )
            .toBe(false);
    ...
    await reset.click();
    for (const monitor of [page, other]) {
        await expect(monitor.getByRole('group', { name: '방송 또는 데모' })).toBeVisible();
        await expect(monitor.getByRole('radio', { name: '실제 방송' })).toBeChecked();
...
test('settings modal applies validated runtime values to all screens and cancels drafts', async ({
    page,
    context,
    request,
}) => {
    ...
    expect((await (await request.get('/health')).json()).source).toMatchObject({
        source: 'mock',
        username: null,
    });
    await open.click();
...
test('stale settings cannot restart a monitor stopped by another screen', async ({
    page,
    request,
}) => {
    await startMock(request);
...
test('development proxy preserves origin for start, settings and refresh', async ({
    page,
    request,
}) => {
    await change(request, 'DELETE', '/account');
    const dev = spawn(
        ...
        {
            cwd: resolve('.'),
            env: { ...process.env, BACKEND_URL: 'http://127.0.0.1:18765' },
            stdio: 'pipe',
        },
    );
    let logs = '';
    ...
    try {
        await expect
            .poll(
                async () => {
                    ...
                    try {
                        return (await request.get('http://127.0.0.1:18766/health')).ok();
                    } catch {
                        return false;
                    }
                },
        ...
        await expect
            .poll(async () => (await (await request.get('/config')).json()).session_id)
            .not.toBe(before);
    } finally {
        ...
        Object.defineProperty(navigator, 'clipboard', {
            ...
            value: {
                writeText: async (text: string) =>
                    Object.defineProperty(window, 'copiedId', { value: text, configurable: true }),
            },
        });
    });
    await page.routeWebSocket('**/ws', (socket) =>
...
test('theme toggle updates the full palette and survives reload without restarting the monitor', async ({
    page,
    request,
}) => {
    await startMock(request);
    ...
    await page.screenshot({
        path: 'test-results/light-theme-settings.png',
        animations: 'disabled',
    });
    await page.getByRole('button', { name: '취소', exact: true }).click();
    await page.reload();
    ...
    await page.reload();
    await expect(
        page.getByRole('button', { name: '라이트 테마로 전환', exact: true }),
    ).toBeVisible();
});

test('font scale is visible, persists, validates saved values and tolerates blocked storage', async ({
    page,
    request,
}) => {
    await startMock(request);
    await page.goto('/');
    const output = page
        .getByRole('group', { name: '댓글 글자 크기', exact: true })
        .locator('output');
...
test('empty view follows server and broadcast states while existing comments remain', async ({
    page,
}) => {
    ...
    const status = {
        ...
        username: 'viewer',
        comment_history_size: 30,
        live: { state: 'unknown', viewers: null, likes: null },
    };
    send!(status);
...
test('uncertain settings can close and only read status until the server finishes', async ({
    page,
    request,
}) => {
    await startMock(request);
...
test('settings request timeout releases the dialog and accepts an already received session boundary', async ({
    page,
}) => {
    let send: (data: object) => void;
...
test('viewport resizing follows the latest feed without moving a reader', async ({ page }) => {
    await page.routeWebSocket('**/ws', (socket) => {
        socket.send(
            JSON.stringify({
                type: 'status',
...
test('reading mode preserves its anchor, bounds unread items and resets across queued session changes', async ({
    ...
    const batch = (from: number, count: number, prefix = 'reading') => {
        for (let i = from; i < from + count; i++)
            send({
                ...
                user: { nickname: `시청자 ${i}`, unique_id: `viewer${i}` },
    ...
    const anchor = () =>
        viewport.evaluate((node) => {
            ...
            const first = [...node.querySelectorAll<HTMLElement>('[data-comment-id]')].find(
    ...
    await expect
        .poll(async () => Math.abs((await anchor()).offset - before.offset))
        .toBeLessThanOrEqual(1);
    await expect(controls).toContainText('보관 중인 새 항목 5개');

    await page.getByRole('button', { name: '댓글 글자 크게', exact: true }).click();
    await expect(page.getByRole('group', { name: '댓글 글자 크기', exact: true })).toContainText(
        '105%',
    );
    await expect.poll(async () => (await anchor()).id).toBe(before.id);
    await expect
    ...
    await page.evaluate(() => {
        ...
        window.cancelAnimationFrame = (id) => {
            if (!frames.delete(id)) originalCancel(id);
        };
    ...
    batch(65, 5);
    await expect
        .poll(() => page.evaluate(() => Reflect.get(window, 'heldMonitorFrames')()))
        .toBe(1);
    await expect(rows.last()).toHaveAttribute('data-comment-id', 'reading-64');
    ...
    await expect
        .poll(() =>
            rows.evaluateAll((nodes) => nodes.map((node) => node.getAttribute('data-comment-id'))),
        )
