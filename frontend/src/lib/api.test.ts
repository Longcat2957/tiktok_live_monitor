...
it.each(['headers', 'body'])('times out while waiting for %s and cleans up its timer', async phase => {
  vi.useFakeTimers();
  ...
  const rejected = expect(requestJson('/config', {}, 50)).rejects.toMatchObject({ name: 'AbortError' });
...
it('cleans up after success and reports HTTP status for the caller', async () => {
  vi.useFakeTimers();
  vi.stubGlobal('fetch', vi.fn(async () => new Response('{"detail":"conflict"}', { status: 409 })));
  expect(await requestJson('/account')).toEqual({ ok: false, status: 409, data: { detail: 'conflict' } });
  expect(vi.getTimerCount()).toBe(0);
});
