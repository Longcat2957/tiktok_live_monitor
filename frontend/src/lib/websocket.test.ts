...
const comment = (id: string): CommentMessage => ({
    ...
    user: { nickname: '민수', unique_id: 'minsu' },
...
describe('message validation', () => {
    it.each([
        ...
        '{"type":"gift"}',
        '{"type":"comment","id":"x"}',
        '{"type":"status","source":"wrong","state":"connected","message":"ok"}',
    ])('ignores malformed payload %s', (raw) => expect(parseMessage(raw)).toBeNull());
    it('accepts a valid comment without changing its content', () =>
        expect(parseMessage(JSON.stringify(comment('a')))).toEqual(comment('a')));
    it('rejects invalid date and empty body', () => {
        expect(
            parseMessage(JSON.stringify({ ...comment('a'), received_at: 'invalid' })),
        ).toBeNull();
...
it('uses same-origin ws and wss', () => {
    expect(websocketUrl({ protocol: 'https:', host: 'monitor.test' })).toBe(
        'wss://monitor.test/ws',
    );
    expect(websocketUrl({ protocol: 'http:', host: 'monitor.test:8000' })).toBe(
        'ws://monitor.test:8000/ws',
    );
});
...
it('validates enriched comments, safe avatars, badges and activities', () => {
    const enriched = {
        ...
        user: {
            ...
            badges: [{ kind: 'fan' as const, level: 12 }],
        },
    };
    ...
    expect(
        appendComment<import('./types').FeedMessage>(
            [enriched],
            activity as import('./types').ActivityMessage,
            1,
        ),
    ).toEqual([activity]);
});
...
it('keeps comment text when optional profile data is invalid and never uses unsafe avatars', () => {
    ...
    for (const avatar_url of [
        ...
        const raw = JSON.stringify({
            ...original,
            user: { ...original.user, avatar_url, badges: 'invalid' },
        });
...
it('retains only valid unique badges in receive order for comments and activities', () => {
    const user = {
        ...
        badges: [
            null,
            { kind: 'unknown', level: 1 },
            { kind: 'fan', level: -1 },
            { kind: 'fan', level: 10001 },
            { kind: 'fan', level: 1.5 },
            { kind: 'fan', level: 12 },
            { kind: 'fan', level: 99 },
            { kind: 'subscriber', level: null },
            { kind: 'subscriber', level: 3 },
        ],
    };
    const expected = {
        ...comment('badges').user,
    ...
    const activity = {
        type: 'activity',
        id: 'activity',
        received_at: comment('badges').received_at,
        user,
        kind: 'follow',
        gift_name: '',
        count: 1,
    ...
    expect(
        parseMessage(
            JSON.stringify({
                ...comment('none'),
                user: { ...user, badges: [{ kind: 'fan', level: -1 }] },
            }),
        ),
    ).toEqual({ ...comment('none'), user: { ...comment('none').user, badges: [] } });
});
...
it('validates live snapshots and preserves unknown counts', () => {
    ...
    expect(
        parseMessage(JSON.stringify({ ...status, live: { ...status.live, viewers: -1 } })),
    ).toBeNull();
});

it('rejects non-string enums without coercing arrays or throwing on malformed objects', () => {
    const status = {
        type: 'status',
        source: 'mock',
        state: 'connected',
        ...
        username: null,
        comment_history_size: 30,
        live: { state: 'live', viewers: null, likes: null },
    };
    const activity = {
        ...comment('activity'),
        type: 'activity',
    ...
    for (const value of [null, ['connected'], { toString: null }, { toString: 'connected' }]) {
        expect(parseMessage(JSON.stringify({ ...status, type: value }))).toBeNull();
        expect(parseMessage(JSON.stringify({ ...status, state: value }))).toBeNull();
    }
    expect(parseMessage(JSON.stringify({ ...status, source: ['mock'] }))).toBeNull();
    expect(
        parseMessage(JSON.stringify({ ...status, live: { ...status.live, state: ['live'] } })),
    ).toBeNull();
    expect(parseMessage(JSON.stringify({ ...activity, kind: ['follow'] }))).toBeNull();

    const receive = vi.fn();
    const stop = connectMonitor('ws://monitor/ws', receive, vi.fn());
    const socket = FakeSocket.instances[0];
    socket.onmessage?.({ data: '{"type":{"toString":null}}' });
    socket.onmessage?.({ data: JSON.stringify(comment('after invalid')) });
    expect(receive).toHaveBeenCalledExactlyOnceWith(comment('after invalid'));
    stop();
});

it('matches backend character limits for emoji comments, names and gifts', () => {
    const user = { nickname: '😀'.repeat(256), unique_id: '😀'.repeat(256) };
    const message = { ...comment('emoji'), user, comment: '😀'.repeat(10000) };
    const activity = {
        ...message,
        type: 'activity',
        kind: 'gift',
        gift_name: '😀'.repeat(256),
        count: 1,
    };
    const parsed = parseMessage(JSON.stringify(message));
    expect(parsed?.type).toBe('comment');
    if (parsed?.type === 'comment') expect(parsed.comment.length).toBe(message.comment.length);
    expect(parseMessage(JSON.stringify(activity))?.type).toBe('activity');
    expect(parseMessage(JSON.stringify({ ...message, comment: message.comment + 'a' }))).toBeNull();
    expect(
        parseMessage(
            JSON.stringify({ ...message, user: { ...user, nickname: user.nickname + 'a' } }),
        ),
    ).toBeNull();
    expect(
        parseMessage(JSON.stringify({ ...activity, gift_name: activity.gift_name + 'a' })),
    ).toBeNull();
});
